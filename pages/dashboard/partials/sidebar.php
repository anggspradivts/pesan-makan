<aside class="w-64 bg-white shadow-lg">
  <div class="p-6 font-bold text-xl text-black">
    Admin Panel
  </div>
  <nav class="mt-8 space-y-2 w-full">
    <a href="dashboard.php?page=index" class="block px-6 py-2 hover:bg-gray-100">Dashboard</a>
    <a href="dashboard.php?page=menu" class="block px-6 py-2 hover:bg-gray-100">Menu</a>
    <a href="dashboard.php?page=orders" class="block px-6 py-2 hover:bg-gray-100">Orders</a>
    <a href="http://localhost/uas.test" class="fa-solid fa-house block px-6 py-2" title="View Site"></a>
    <a href="../auth/logout.php" class="block px-6 py-2 text-red-600 hover:bg-red-100">Logout</a>
  </nav>
</aside>